...
internal class RolesProvider
{
    ...
    internal RolesProvider(IDatabaseConnection databaseConnection)
    {
        _databaseConnectionFactory = databaseConnection;
    }
    ...
    public async Task<IdentityResult> CreateAsync(ApplicationRole role, CancellationToken cancellationToken) {
        ...
        await using (var sqlConnection = await _databaseConnectionFactory.CreateConnectionAsync()) {
            rowsInserted = await sqlConnection.ExecuteAsync(sql, new {
                role.Id,
                role.Name,
                role.NormalizedName,
    ...
    public async Task<IdentityResult> UpdateAsync(ApplicationRole role) {
        var command = 
        $@"
        UPDATE [{_databaseConnectionFactory.DbSchema}].[Roles]
        SET 
            Name = @Name, 
        ...
        await using (var sqlConnection = await _databaseConnectionFactory.CreateConnectionAsync())
        {
            ...
            if (role?.Claims?.Count > 0) {
                ...
                $@"
                DELETE FROM [{_databaseConnectionFactory.DbSchema}].[RoleClaims]
                WHERE 
                    RoleId = @RoleId
                ";
                ...
                $@"
                INSERT INTO [{_databaseConnectionFactory.DbSchema}].[RoleClaims] 
    ...
    public async Task<IdentityResult> DeleteAsync(ApplicationRole role) {
        var command = 
        $@"
        DELETE FROM [{_databaseConnectionFactory.DbSchema}].[Roles]
        WHERE 
            Id = @Id
        ";
